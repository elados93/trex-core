...
All nodes are identified by a unique MAC address, however there are some differences. We support 3 types of nodes: *Normal, Shared Namespace and Bird nodes*. 

====== Normal Node

These are the default type of nodes, creating a namespace and a veth pair. These nodes are configured by ipv4 and a default gateway.
...
These are nodes with pre-created namespace, they only creating the veth pair on the existing namespace. That way you can create multiple veths for namespace. These nodes are configured by ipv4 and a subnet mask.
...
Bird node is a specific case of shared namespace node. Bird namespace is created once running TRex with "--bird-server" flag, in that way they are similar to shared namespace. To learn more on the usage of bird with TRex see: link:trex_stateless.html#_bird_intgration_beta[Bird Integration].

NOTE: You cannot set is_bird = True and shared_ns = "my-ns", they are mutual exclusive arguments.

Examples with bird nodes can be found link:trex_stateless.html#_tutorial_simple_bird_node_adding[here].

====== Summary of node types differences

.Types of nodes differences
|===
|Node Type | Node : veth ratio | Namespace creation | Set ipv4  
...
|Shared Namespace
|1 : Many
|Pre-created, name determined by user
...
pybird_c.check_protocols_up(['my_bgp1, my_bgp2'])                   <5>
pybird_c.release()                                                  <6>
pybird_c.disconnect()
c.disconnect()
----
...
<5> ensure bird works on our protocols
<6> exiting gracefully PyBirdClient
